...
public class MainActivityTest extends ActivityInstrumentationTestCase2<MainActivity> {
    ...
    public void testOnItems() throws Throwable {
        ...
        runTestOnUiThread(new Runnable() {
            @Override
            public void run() {
                substitutePresenter(MainActivity.class, mainPresenterMock = mockPresenter());
            }
        });
    ...
    private void substitutePresenter(Class viewClass, final Presenter mockPresenter) {
        PresenterManager mockManager = mock(PresenterManager.class);
        when(mockManager.provide(any(viewClass), any(Bundle.class))).then(new Answer<Object>() {
            @Override
            public Object answer(InvocationOnMock invocation) throws Throwable {
