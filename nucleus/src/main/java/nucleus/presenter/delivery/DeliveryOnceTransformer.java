...
public final class DeliveryOnceTransformer<ViewType, T> implements Observable.Transformer<T, Delivery<ViewType, T>> {

    private final Observable<ViewType> view;

    public DeliveryOnceTransformer(Observable<ViewType> view) {
        this.view = view;
    }
    ...
    public Observable<Delivery<ViewType, T>> call(final Observable<T> observable1) {
        return Observable
            .combineLatest(view, observable1.materialize().first(), new Func2<ViewType, Notification<T>, Delivery<ViewType, T>>() {
                @Override
